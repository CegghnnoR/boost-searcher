...
namespace ns_searcher {
    ...
    public:
        ...
        void Search(const std::string& query, std::string& json_string) {
            ...
            // 2.触发，合并：根据分出的各个词，查倒排，同时合并所有的InvertedElem
            // std::vector<ns_index::InvertedElem> inverted_list_all;
            std::vector<InvertedElemPrint> inverted_list_all;
            std::unordered_map<uint64_t, InvertedElemPrint> tokens_map;

            for (std::string word : words) {
                boost::to_lower(word); // 全部转小写，以便于忽略大小写
                std::vector<ns_index::InvertedElem>* inverted_list = index->GetInvertedList(word);
        ...
        std::string GetDesc(const std::string& html_content, const std::string& word) {
            // 找到word在html_content中的首次出现，然后往前找50个字节，往后找100字节，截取出这部分内容
